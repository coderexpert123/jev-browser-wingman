...
export function buildPrompt(task: BenchTask, route: 'playwright' | 'wingman'): string {
  ...
  if (route === 'wingman') {
    prompt +=
      ' Prefer the wingman_do tool: call it with the goal and the values. If it returns fallback, ambiguous, blocked or login, finish the task with the Playwright tools.';
  }
  return prompt;
}
