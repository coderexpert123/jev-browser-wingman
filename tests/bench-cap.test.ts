...
test('refuses a cap above the operator ceiling', async () => {
  ...
  assert.equal(exit, 2);
  assert.match(out, /BENCH-REFUSED: cap above operator ceiling \(5 per run, 10 per phase\)/);
  assert.equal(runner.calls(), 0);
});
...
test('a harness without the abort check would run all 16', async () => {
  ...
  assert.equal(parsed.runs.length, 16);
});
